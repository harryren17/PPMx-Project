...
master <- master %>% 
  mutate(
    ...
    #create 1 week rolling sum for errors
...
master_model <- subset(master, select = c(fcomp3, comp3_age,
                                        volt_rmean_48, rotate_rmean_48,
                                        pressure_rmean_48, vibe_rmean_48,
                                        error1_rsum_168, error2_rsum_168,
                                        error3_rsum_168, error4_rsum_168,
                                        error5_rsum_168))

#split dataset into train/test dfs
train_index <-  createDataPartition(master_model$fcomp3, 
                                    p = .50, 
                                    list = FALSE,
                                    times = 1 )
